...
def check_security_tamper():
    ...
    checksums_file = "security_checksums.json"
    
    try:
        current_checksums = tamper_check.compute_config_checksum(critical_files)
        
        if os.path.exists(checksums_file):
            with open(checksums_file, "r") as f:
                stored_checksums = json.load(f)
            
            changed_files = tamper_check.verify_checksums(stored_checksums, current_checksums)
            
            if changed_files:
                for file in changed_files:
                    audit.audit_security_event("config_tamper_detected", {
                        "file": file,
                        "timestamp": datetime.utcnow().isoformat()
                    })
                log.warning(f"Tamper detected in {len(changed_files)} files: {changed_files}")
            else:
                log.info("✅ Security tamper check: No changes detected")
        else:
            with open(checksums_file, "w") as f:
                json.dump(current_checksums, f, indent=2)
            log.info(f"✅ Initialized security checksums baseline for {len(current_checksums)} files")
            audit.audit_security_event("security_baseline_initialized", {
                "files": list(current_checksums.keys()),
                "count": len(current_checksums)
            })
